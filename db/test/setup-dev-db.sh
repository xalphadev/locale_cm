...
for n in 0003 0004 0005 0007 0008 0009 0010 0011 0012 0013 0014 0015 0016; do cp "$MIG/${n}"*.sql "$STUB/$n.sql"; done
...
for t in loop supply lifecycle seed-interactive seed-agents seed-venues seed-reviews seed-events seed-deals seed-media seed-profile; do $PSQL -d $DB -q -f "$ROOT/db/test/$t.sql" >/dev/null 2>&1; done
