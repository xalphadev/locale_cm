...
set -euo pipefail
echo "── locale: applying migrations (real PostGIS) ──"
for f in /migrations/0*.sql; do
  echo "   apply $(basename "$f")"
  psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" -q -f "$f"
done
echo "── locale: schema ready ($(psql -tA --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" -c "SELECT count(*) FROM information_schema.tables WHERE table_schema='public'") tables) ──"
