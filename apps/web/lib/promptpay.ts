// PromptPay QR payload (EMVCo / Bank of Thailand standard) with the amount embedded, so the tenant scans and
// the transfer amount is pre-filled. Pure string maths — no money moves; the QR just addresses the owner's own
// PromptPay (phone / national id). Pair with a QR renderer (qrcode). Copied from apps/consumer/lib/promptpay.ts.
...
/** EMVCo payload string for the owner's PromptPay + amount (THB). Returns '' if no valid target. */
