...
export default async function StayHome() {
  ...
  const [rp] = booking ? await q<any>(`SELECT count(*) FILTER (WHERE status='new')::int newn FROM stay_maintenance WHERE place_id=$1 AND deleted_at IS NULL`, [acc.place_id]) : [];
  const tiles: { href: string; icon: string; label: string; stat: string; hot?: boolean; badge?: number }[] = [
    ...
    ...(booking ? [{ href: '/merchant/repairs', icon: 'ruler', label: 'แจ้งซ่อม', stat: (rp?.newn || 0) > 0 ? `${rp.newn} เรื่องใหม่` : 'งานซ่อม', hot: (rp?.newn || 0) > 0, badge: rp?.newn || undefined }] : []),
