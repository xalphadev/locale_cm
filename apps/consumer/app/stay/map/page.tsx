...
export default async function StayMap({ searchParams }: { searchParams: Record<string, string> }) {
  ...
  const { mode, kind, sort, am, fr, qtext, pr, beds, gender, focus, pins, placeList, activeCount, href, dateQs } = d;
  return (
    <StayMapView pins={pins} focus={focus} full backHref={href({}, '/stay/search')} qs={dateQs}>
      ...
      <StayFilterSheet mode={mode} q={qtext} kind={kind} sort={sort} am={am} fr={fr} pr={pr} beds={beds} gender={gender} count={activeCount} from={d.fromQ as string} to={d.toQ as string} basePath="/stay/map" />
    </StayMapView>
  );
}
