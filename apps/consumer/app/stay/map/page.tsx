...
export default async function StayMap({ searchParams }: { searchParams: Record<string, string> }) {
  ...
  const { mode, kind, sort, am, fr, qtext, pr, cap, focus, pins, placeList, activeCount, href, dateQs } = d;
  return (
    <StayMapView pins={pins} focus={focus} full backHref={href({}, '/stay')} qs={dateQs}>
      <div className="mapcount-pill">พบ {placeList.length} ที่พัก</div>
      <StayFilterSheet mode={mode} q={qtext} kind={kind} sort={sort} am={am} fr={fr} pr={pr} cap={cap} count={activeCount} from={d.fromQ as string} to={d.toQ as string} basePath="/stay/map" />
    </StayMapView>
  );
}
