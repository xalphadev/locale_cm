import type { CSSProperties } from 'react';

// Hand-built stroke icon set (24×24, currentColor) — NO emoji anywhere in the app.
const P: Record<string, JSX.Element> = {
  search: <><circle cx="11" cy="11" r="7" /><path d="m20 20-3.2-3.2" /></>,
  bell: <><path d="M6 9a6 6 0 0 1 12 0c0 6 2.5 7 2.5 7h-17S6 15 6 9Z" /><path d="M10.3 20.5a2 2 0 0 0 3.4 0" /></>,
  flag: <><path d="M5 21V4" /><path d="M5 4h11l-2 3 2 3H5" /></>,
  store: <><path d="M4 9.5 5.5 4h13L20 9.5" /><path d="M4 9.5h16v1.7a2.4 2.4 0 0 1-4.8 0 2.4 2.4 0 0 1-4.8 0 2.4 2.4 0 0 1-4.8 0Z" /><path d="M5.5 12v8h13v-8" /></>,
  send: <><path d="M21 3 10.5 13.5" /><path d="M21 3 14.5 21l-4-8.5L2 8.5 21 3Z" /></>,
  user: <><circle cx="12" cy="8" r="3.6" /><path d="M5 20a7 7 0 0 1 14 0" /></>,
  pin: <><path d="M19 10c0 5.5-7 11-7 11s-7-5.5-7-11a7 7 0 0 1 14 0Z" /><circle cx="12" cy="10" r="2.5" /></>,
  chevR: <path d="m9 6 6 6-6 6" />,
  chevD: <path d="m6 9 6 6 6-6" />,
  back: <><path d="M19 12H5" /><path d="m11 19-7-7 7-7" /></>,
  star: <path d="m12 3 2.6 5.3 5.9.9-4.3 4.1 1 5.8L12 16.9 6.8 19.2l1-5.8L3.5 9.2l5.9-.9L12 3Z" />,
  bookmark: <path d="M18 21l-6-3.6L6 21V5a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2Z" />,
  clock: <><circle cx="12" cy="12" r="8.5" /><path d="M12 7.5V12l3 1.8" /></>,
  phone: <path d="M21 16.5v2.6a2 2 0 0 1-2.2 2 18 18 0 0 1-7.8-2.8 17.6 17.6 0 0 1-5.4-5.4A18 18 0 0 1 2.8 5.1 2 2 0 0 1 4.8 3h2.6a2 2 0 0 1 2 1.7c.1.9.3 1.7.6 2.5a2 2 0 0 1-.5 2.1L8.4 12a16 16 0 0 0 5.5 5.5l1.6-1.1a2 2 0 0 1 2.1-.5c.8.3 1.6.5 2.5.6a2 2 0 0 1 1.7 2Z" />,
  globe: <><circle cx="12" cy="12" r="8.5" /><path d="M3.5 12h17M12 3.5a13 13 0 0 1 0 17 13 13 0 0 1 0-17Z" /></>,
  chat: <path d="M20 15a2 2 0 0 1-2 2H8l-4 3.5V6a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2Z" />,
  directions: <path d="M11 3 3 11.5l8 8.5 8-8.5L11 3Zm0 6v3h4" />,
  sparkles: <path d="m12 3 1.7 4.6L18 9l-4.3 1.4L12 15l-1.7-4.6L6 9l4.3-1.4L12 3Z" />,
  ticket: <path d="M4 7a1.5 1.5 0 0 1 1.5-1.5h13A1.5 1.5 0 0 1 20 7a2 2 0 0 0 0 4 1.5 1.5 0 0 1 0 3 2 2 0 0 0 0 4 1.5 1.5 0 0 1-1.5 1.5h-13A1.5 1.5 0 0 1 4 17a2 2 0 0 0 0-4 1.5 1.5 0 0 1 0-3 2 2 0 0 0 0-4Z" />,
  wallet: <><path d="M4 8a2 2 0 0 1 2-2h12v3" /><path d="M4 8v8a2 2 0 0 0 2 2h13v-4" /><path d="M19 11a2 2 0 0 0 0 4h2v-4Z" /></>,
  compass: <><circle cx="12" cy="12" r="8.5" /><path d="m15 9-2 5-4 2 2-5 4-2Z" /></>,
  gift: <><path d="M5 11h14v9H5z" /><path d="M3 7.5h18V11H3zM12 7.5V20" /><path d="M12 7.5C12 5 10.5 4 9 4.5S9 7.5 12 7.5ZM12 7.5C12 5 13.5 4 15 4.5S15 7.5 12 7.5Z" /></>,
  check: <path d="m20 7-11 11-5-5" />,
  // categories
  coffee: <><path fill="currentColor" stroke="none" d="M4 7.5h11v5a5.5 5.5 0 0 1-11 0z" /><path fill="currentColor" stroke="none" d="M15 8.6h1.8a2.8 2.8 0 0 1 0 5.6H15z" /><rect x="4.5" y="19.4" width="11" height="1.9" rx=".95" fill="currentColor" stroke="none" /></>,
  bowl: <><path fill="currentColor" stroke="none" d="M3 10.6h18a9 9 0 0 1-18 0z" /><rect x="2.6" y="8.5" width="18.8" height="1.8" rx=".9" fill="currentColor" stroke="none" /></>,
  flame: <path fill="currentColor" stroke="none" d="M12 2.5c3.2 3.4 5.5 6 5.5 9.5a5.5 5.5 0 0 1-11 0c0-1.7.7-3 1.9-4.2.3 1.1 1.2 1.9 2.1 2.1C11 8 11 5.2 12 2.5z" />,
  cake: <><path fill="currentColor" stroke="none" d="M4 12c0-2 2-3.3 4.2-3.3h7.6C18 8.7 20 10 20 12v.7H4z" /><path fill="currentColor" stroke="none" d="M4 14h16v4a2.4 2.4 0 0 1-2.4 2.4H6.4A2.4 2.4 0 0 1 4 18z" /><circle cx="12" cy="4.4" r="1.25" fill="currentColor" stroke="none" /><rect x="11.3" y="5.4" width="1.4" height="3.3" rx=".7" fill="currentColor" stroke="none" /></>,
  landmark: <><path fill="currentColor" stroke="none" d="M12 2.6 21 7.2v1.7H3V7.2z" /><rect x="4.6" y="10.1" width="2.3" height="6.5" rx=".5" fill="currentColor" stroke="none" /><rect x="10.85" y="10.1" width="2.3" height="6.5" rx=".5" fill="currentColor" stroke="none" /><rect x="17.1" y="10.1" width="2.3" height="6.5" rx=".5" fill="currentColor" stroke="none" /><rect x="3" y="17.9" width="18" height="2.4" rx=".7" fill="currentColor" stroke="none" /></>,
  palette: <path fill="currentColor" stroke="none" d="M12 3a9 9 0 0 0-.5 18c1 .06 1.6-.85 1.5-1.65-.12-1 .72-1.85 1.7-1.85H16a4 4 0 0 0 4-4.1C19.7 7.5 16.3 3 12 3z" />,
  flower: <g fill="currentColor" stroke="none"><ellipse cx="12" cy="6.4" rx="2.2" ry="3.3" /><ellipse cx="12" cy="6.4" rx="2.2" ry="3.3" transform="rotate(72 12 12)" /><ellipse cx="12" cy="6.4" rx="2.2" ry="3.3" transform="rotate(144 12 12)" /><ellipse cx="12" cy="6.4" rx="2.2" ry="3.3" transform="rotate(216 12 12)" /><ellipse cx="12" cy="6.4" rx="2.2" ry="3.3" transform="rotate(288 12 12)" /><circle cx="12" cy="12" r="2" /></g>,
  dumbbell: <path d="M6.5 7v10M18 7v10M4 9.5v5M20.5 9.5v5M6.5 12H18" />,
  calendar: <><rect x="3.5" y="5" width="17" height="16" rx="2" /><path d="M3.5 9.5h17M8 3v4M16 3v4" /></>,
  map: <><path d="m9 4 6 2 5.4-2.2A.5.5 0 0 1 21 4.3v13.4a1 1 0 0 1-.6.9L15 21l-6-2-5.4 2.2A.5.5 0 0 1 3 20.7V7.3a1 1 0 0 1 .6-.9Z" /><path d="M9 4v15M15 6v15" /></>,
  locate: <><circle cx="12" cy="12" r="6.5" /><circle cx="12" cy="12" r="1.6" fill="currentColor" /><path d="M12 2.5v2.5M12 19v2.5M2.5 12H5M19 12h2.5" /></>,
  x: <path d="M18 6 6 18M6 6l12 12" />,
  play: <path d="M7 4.5v15l13-7.5z" />,
  feed: <><rect x="4" y="4" width="16" height="16" rx="2.5" /><path d="M8 9h8M8 13h8M8 17h5" /></>,
  heart: <path d="M12 20s-7-4.4-9.4-8.8A4.5 4.5 0 0 1 12 6a4.5 4.5 0 0 1 9.4 5.2C19 15.6 12 20 12 20Z" />,
  share: <><path d="M4 12v7a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-7" /><path d="M12 3v13M8 7l4-4 4 4" /></>,
  dots: <><circle cx="5" cy="12" r="1.6" /><circle cx="12" cy="12" r="1.6" /><circle cx="19" cy="12" r="1.6" /></>,
  tag: <path fill="currentColor" stroke="none" d="M3.5 11.4V5a1.5 1.5 0 0 1 1.5-1.5h6.4c.4 0 .78.16 1.06.44l8 8a1.5 1.5 0 0 1 0 2.12l-6.4 6.4a1.5 1.5 0 0 1-2.12 0l-8-8a1.5 1.5 0 0 1-.44-1.06z" />,
  bed: <><path fill="currentColor" stroke="none" d="M3.1 7.2a1.1 1.1 0 0 1 2.2 0v4.2h13.5V10a3.2 3.2 0 0 1 3.2 3.2V19a1.1 1.1 0 0 1-2.2 0v-1.7H5.3V19a1.1 1.1 0 0 1-2.2 0z" /><path fill="currentColor" stroke="none" d="M7 9.1h5a1.9 1.9 0 0 1 1.9 1.9v.4H5.1V11A1.9 1.9 0 0 1 7 9.1z" /></>,
  ruler: <><path d="m16 2 6 6L8 22l-6-6Z" /><path d="m7.5 10.5 2 2M11 7l2 2M14.5 3.5l2 2M4 14l2 2" /></>,
  sofa: <><path d="M4 11V8a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v3" /><path d="M2 13a2 2 0 0 1 4 0v3h12v-3a2 2 0 0 1 4 0v5H2Z" /></>,
  users: <><circle cx="9" cy="8" r="3" /><path d="M3 20a6 6 0 0 1 12 0" /><path d="M16 6a3 3 0 0 1 0 6M21 20a6 6 0 0 0-4-5.6" /></>,
  clock2: <><circle cx="12" cy="12" r="8.5" /><path d="M12 7.5V12l3 1.8" /></>,
  sort: <><path d="M7 4v16M7 20l-3-3M7 20l3-3" /><path d="M17 20V4M17 4l-3 3M17 4l3 3" /></>,
};

export function Icon({ n, size = 22, fill = 'none', style, className }:
  { n: string; size?: number; fill?: string; style?: CSSProperties; className?: string }) {
  return (
    <svg viewBox="0 0 24 24" width={size} height={size} fill={fill} stroke="currentColor"
      strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={style} className={className}
      aria-hidden="true">{P[n] ?? P.compass}</svg>
  );
}

export const CAT_ICON: Record<string, string> = {
  eat: 'bowl', see: 'landmark', do: 'palette',
  cafe: 'coffee', restaurant: 'bowl', street_food: 'flame', dessert: 'cake', bar: 'coffee',
  temple: 'landmark', viewpoint: 'pin', market: 'bowl', museum: 'landmark',
  cooking_class: 'flame', spa: 'flower', muay_thai: 'dumbbell', workshop: 'palette',
  dorm: 'bed', apartment: 'bed', homestay: 'bed', hotel: 'bed', guesthouse: 'bed',
};
export const KIND_ICON: Record<string, string> = {
  festival: 'sparkles', market: 'bowl', performance: 'palette', workshop: 'palette', seasonal: 'calendar',
};
